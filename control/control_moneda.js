//resp_archivo
    ...
    function btn_mostrar_registro()
    {
     ...
     $.ajax({
      type: 'POST',
      url:'../vista/vista_moneda.php',
      data: obj,
       beforeSend: function(objeto){
      },
      success:function(data){
        $('#panel_registro_moneda').html(data).fadeIn('slow');
      }
    });
    }

    function btn_registro_moneda(){
      
         var moneda = $('#moneda').val(); 
         var ob = {moneda : moneda};
    
         if(moneda !='' ){ 

                $.ajax({
                type: 'POST',
                url:'../modelo/modelo_registrar_moneda.php',
                data: ob,
                beforeSend: function(objeto){
                ...
                { 
                 //alert(data);
                 $('#resultado_registro_moneda').html(data);
                // setTimeout(limpiar_moneda,1000);

                 
                  $('#moneda').val('');
                 
                  setTimeout(function(){
                    $('#resultado_registro_moneda').html('');
                  },1000);
    ...
    function limpiar_moneda()
    {
         $('#resultado_registro_moneda').html('');
         $('#resp_img_moneda').html('');
         $('#img_moneda').val('');
    }
  ...
  function cargar_datos_buscar(page)
  {
    ...
    if(txt_buscar==''){
      $('#resp_busqueda_moneda').html('<label style="color:red;"> !!Escribe en el campo para Buscar!! </label>');
    }

    else{
    $('#resp_busqueda_moneda').html('');
    ...
    $.ajax({
      url:'../modelo/modelo_buscar_moneda.php',
      data: parametros,
       beforeSend: function(objeto){
       $('#loader').html('cargando .... ');
      },
