function btn_mostrar_registro()
{
    ...
    $.ajax({
        type: 'POST',
        url:'../vista/vista_color_prenda.php',
        data: obj,
        beforeSend: function(objeto){
        },
        success:function(data){
            $('#panel_registro_color_prenda').html(data).fadeIn('slow');
        }
    });
}
...
function btn_registro_color_prenda(){

    var nombre_color = $('#nombre_color').val();
    var cod_color = $('#cod_color').val();
    var ob = {nombre_color : nombre_color,
             cod_color:cod_color};
    ...
    if(nombre_color !='' ){ 
        ...
        $.ajax({
            type: 'POST',
            url:'../modelo/modelo_registrar_color_prenda.php',
            data: ob,
            beforeSend: function(objeto){
            ...
            { 
                //alert(data);
                $('#resultado_registro_color_prenda').html(data);
                // setTimeout(limpiar_color_prenda,1000);


                $('#nombre_color').val('');

                setTimeout(function(){
                    $('#resultado_registro_color_prenda').html('');
                },1000);
        ...
        if (color_prenda ==''){ $('#color_prenda').focus();
                               var res ='<label style="color:red;"> Debe llenar el campo de color_prenda </label>';
                               $('#resp_color_prenda').html(res);}} 

} 
function limpiar_color_prenda()
{
    $('#resultado_registro_color_prenda').html('');
    $('#resp_img_color_prenda').html('');
    $('#img_color_prenda').val('');
}
...
function cargar_datos_buscar(page)
{
    ...
    if(txt_buscar==''){
        $('#resp_busqueda_color_prenda').html('<label style="color:red;"> !!Escribe en el campo para Buscar!! </label>');
    }

    else{
        $('#resp_busqueda_color_prenda').html('');
        ...
        $.ajax({
            url:'../modelo/modelo_buscar_color_prenda.php',
            data: parametros,
            beforeSend: function(objeto){
                $('#loader').html('cargando .... ');
            },
