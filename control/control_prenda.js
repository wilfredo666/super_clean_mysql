...
function btn_mostrar_registro()
{
    ...
    $.ajax({
        type: 'POST',
        url:'../vista/vista_prenda.php',
        data: obj,
        beforeSend: function(objeto){
        },
        success:function(data){
            $('#panel_registro_prenda').html(data).fadeIn('slow');
        }
    });
}

function btn_registro_prenda(){
    ...
    if(prenda !='' && precio !='' && tipo_prenda !='' && moneda !='' ){ 

        $.ajax({
            type: 'POST',
            url:'../modelo/modelo_registrar_prenda.php',
            data: ob,
            beforeSend: function(objeto){
            ...
            { 
                //alert(data);
                $('#resultado_registro_prenda').html(data);
                // setTimeout(limpiar_prenda,1000);
                ...
                setTimeout(function(){
                    $('#resultado_registro_prenda').html('');
                },1000);
        ...
        if (prenda ==''){ $('#prenda').focus();
                         var res ='<label style="color:red;"> Debe llenar el campo de prenda </label>';
                         $('#resp_prenda').html(res);}
        if (portada ==''){ $('#portada').focus();
                          var res ='<label style="color:red;"> Debe llenar el campo de portada </label>';
                          $('#resp_portada').html(res);}
        if (precio ==''){ $('#precio').focus();
                         var res ='<label style="color:red;"> Debe llenar el campo de precio </label>';
                         $('#resp_precio').html(res);}
...
function limpiar_prenda()
{
    $('#resultado_registro_prenda').html('');
    $('#resp_img_prenda').html('');
    $('#img_prenda').val('');
}
...
function cargar_datos_buscar(page)
{
    ...
    if(txt_buscar==''){
        $('#resp_busqueda_prenda').html('<label style="color:red;"> !!Escribe en el campo para Buscar!! </label>');
    }

    else{
        $('#resp_busqueda_prenda').html('');
        ...
        $.ajax({
            url:'../modelo/modelo_buscar_prenda.php',
            data: parametros,
            beforeSend: function(objeto){
                $('#loader').html('cargando .... ');
            },
