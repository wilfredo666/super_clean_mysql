...
<?php
    ...
    $count_query   = pg_query("SELECT count(*) AS numrows FROM tipo_prenda");
    if ($row= pg_fetch_array($count_query)){ $numrows = $row['numrows'];}
    $total_pages = ceil($numrows/$per_page);
    $reload = '../vista/vista_listar_tipo_prenda.php';

    //consulta principal para recuperar los datos
    $query = pg_query("SELECT * FROM tipo_prenda order by id_tipo_prenda DESC OFFSET $offset LIMIT $per_page");
