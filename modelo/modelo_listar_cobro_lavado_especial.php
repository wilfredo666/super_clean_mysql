...
<?php
    ...
    $count_query   = pg_query("SELECT count(*) AS numrows FROM cobro_lavado_especial");
    if ($row= pg_fetch_array($count_query)){ $numrows = $row['numrows'];}
    $total_pages = ceil($numrows/$per_page);
    $reload = '../vista/vista_listar_cobro_lavado_especial.php';

    //consulta principal para recuperar los datos
    $query = pg_query("SELECT * FROM cobro_lavado_especial order by id_cobro_lavado_especial DESC OFFSET $offset LIMIT $per_page");
