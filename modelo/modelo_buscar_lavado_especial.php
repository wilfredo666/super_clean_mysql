...
<?php
    ...
    ?>
    <a href='javascript:void(0);' onclick='btn_lavados_especiales();'> Volver </a>
    <?php
    ...
    $sql_numrows = pg_query("SELECT DISTINCT codigo FROM lavado_especial WHERE codigo ILIKE '%$txt_buscar%' OR cliente ILIKE '%$txt_buscar%' OR prenda ILIKE '%$txt_buscar%' LIMIT 1");
    $numrows =  0;
    while ($row_numrows = pg_fetch_array($sql_numrows)) {
    $numrows++;
    }
    ...
    $sql_le = pg_query("SELECT DISTINCT codigo  FROM lavado_especial WHERE codigo ILIKE '%$txt_buscar%' OR cliente ILIKE '%$txt_buscar%' OR prenda ILIKE '%$txt_buscar%' OFFSET $offset LIMIT $per_page");
