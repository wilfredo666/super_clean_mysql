...
<?php
    ...
    $count_query   = pg_query("SELECT count(*) AS numrows FROM moneda WHERE moneda ILIKE '%$txt_buscar%' ");
    if ($row= pg_fetch_array($count_query)){ $numrows = $row['numrows'];}
    $total_pages = ceil($numrows/$per_page);
    $reload = '../vista/vista_listar_moneda.php';

    //consulta principal para recuperar los datos
    $query = pg_query("SELECT * FROM moneda WHERE moneda LIKE '%$txt_buscar%' ORDER BY id_moneda DESC OFFSET $offset LIMIT $per_page");
    
    if ($numrows>0){
      ?>
    <a href='javascript:void(0);' onclick='btn_moneda();'> Volver </a>
