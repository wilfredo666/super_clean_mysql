...
<?php
    ...
    $count_query   = pg_query("SELECT count(*) AS numrows FROM forma WHERE forma ILIKE '%$txt_buscar%' ");
    if ($row= pg_fetch_array($count_query)){ $numrows = $row['numrows'];}
    $total_pages = ceil($numrows/$per_page);
    $reload = '../vista/vista_listar_forma_prenda.php';

    //consulta principal para recuperar los datos
    $query = pg_query("SELECT * FROM forma WHERE forma LIKE '%$txt_buscar%' ORDER BY id_forma DESC OFFSET $offset LIMIT $per_page");
    
    if ($numrows>0){
      ?>
    <a href='javascript:void(0);' onclick='btn_forma_prenda();'> Volver </a>
    ...
    <td align='center'>
     ...
     <button type='button' class='btn btn-default btn-xs' onclick="btn_opciones('<?php echo $ID_forma_prenda;?>');"data-toggle='modal' data-target='#myModal_opciones'> ... </button></td>
     </tr>
        <?php
      }
      ?>
