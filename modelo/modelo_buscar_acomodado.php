  <a href='javascript:void(0);' onclick='btn_acomodado();'> Volver </a>
<?php
    ...
    $count_query   = pg_query("SELECT count(*) AS numrows FROM acomodado WHERE cliente ILIKE '%$txt_buscar%' OR codigo_tikeo ILIKE '%$txt_buscar%' OR codigo_ot ILIKE '%$txt_buscar%'");
    if ($row= pg_fetch_array($count_query)){ $numrows = $row['numrows'];}
    $total_pages = ceil($numrows/$per_page);
    $reload = '../vista/vista_listar_acomodado.php';

    //consulta principal para recuperar los datos
    $query = pg_query("SELECT * FROM acomodado WHERE cliente ILIKE '%$txt_buscar%' OR codigo_tikeo ILIKE '%$txt_buscar%' OR codigo_ot ILIKE '%$txt_buscar%' ORDER BY id_acomodado DESC OFFSET $offset LIMIT $per_page");
