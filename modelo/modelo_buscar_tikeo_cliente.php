<a href='javascript:void(0);' onclick='btn_tikeo_cliente();'> Volver </a>
<?php
    ...
    $count_query   = pg_query("SELECT count(*) AS numrows FROM tikeo_cliente WHERE cliente ILIKE '%$txt_buscar%' OR codigo_barras ILIKE '%$txt_buscar%'");
    if ($row= pg_fetch_array($count_query)){ $numrows = $row['numrows'];}
    $total_pages = ceil($numrows/$per_page);
    $reload = '../vista/vista_listar_tikeo_cliente.php';

    //consulta principal para recuperar los datos
    $query = pg_query("SELECT * FROM tikeo_cliente WHERE cliente ILIKE '%$txt_buscar%' OR codigo_barras ILIKE '%$txt_buscar%' ORDER BY id_tikeo_cliente DESC OFFSET $offset LIMIT $per_page");
