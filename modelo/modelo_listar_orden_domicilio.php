...
<?php
    ...
    $count_query   = pg_query("SELECT count(*) AS numrows FROM orden_domicilio");
    if ($row= pg_fetch_array($count_query)){ $numrows = $row['numrows'];}
    $total_pages = ceil($numrows/$per_page);
    $reload = '../vista/vista_listar_orden_domicilio.php';

    //consulta principal para recuperar los datos
    $query = pg_query("SELECT * FROM orden_domicilio order by id_orden_domicilio DESC OFFSET $offset LIMIT $per_page");
